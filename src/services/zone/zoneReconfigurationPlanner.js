...
function actionInvolvesZone(action, zoneId) {
  ...
  if (type === 'assign_orphan_row') {
    return action.toZoneId === zoneId;
  }
  return false;
}
...
function filterLLMActions(actions, zoneRowMap, neverMerge) {
  ...
  const filtered = actions.filter(a => {
    ...
    if (a.type === 'assign_orphan_row') {
      // Orphan recovery actions are always valid — they were generated by Phase 0
      // with colour compatibility already checked
      return !!getZoneById(a.toZoneId);
    }
...
export function buildMutatedZoneRowMap(initialMap, actions) {
  ...
  for (const action of actions) {
    if (action.type === 'reallocate_row') {
      ...
      // Add orphaned row to target zone (no source to remove from)
      const rowId = typeof action.rowNumber === 'number' ? `R${action.rowNumber}` : String(action.rowNumber);
      const toRows = map.get(action.toZoneId) || [];
      if (!toRows.includes(rowId)) toRows.push(rowId);
      map.set(action.toZoneId, toRows);
...
export async function generateReconfigurationPlan(report, options = {}) {
  ...
  // zone row map so the solver can allocate them, AND emit actions so
  // the apply handler persists the recovery in zone_allocations.
  ...
  // Fetch layout settings early so Phase 0 + solver both use the correct colour order
  let layoutSettings = { colourOrder: 'whites-top' };
  try {
    layoutSettings = await getCellarLayoutSettings(cellarId);
  } catch (err) {
    logger.warn('Reconfig', `Failed to fetch layout settings for Phase 0: ${err.message}`);
  }

  const orphanActions = [];
  if (orphanedRows.length > 0) {
    logger.info('Reconfig', `Recovered ${orphanedRows.length} orphaned row(s): ${orphanedRows.join(', ')}`);
    // Pre-fetch colour ranges once using actual colourOrder
    ...
    for (const orphan of orphanedRows) {
      ...
      if (bestZoneId) {
        ...
        // Emit a persistable action so the apply handler writes it to zone_allocations
        orphanActions.push({
          type: 'assign_orphan_row',
          toZoneId: bestZoneId,
          rowNumber: orphanNum,
          reason: `Recovered orphaned R${orphanNum} — not assigned to any zone`
        });
  ...
  const planResult = {
    ...
    // Prepend orphan recovery actions so they are applied first (before any row moves)
    actions: [...orphanActions, ...patchedActions]
  };
