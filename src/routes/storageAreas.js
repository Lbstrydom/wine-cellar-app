...
router.post('/', async (req, res) => {
  try {
    ...
    // Validate input
    if (!name || !storage_type || !temp_zone) {
      return res.status(400).json({
        error: 'Missing required fields: name, storage_type, temp_zone'
      });
    }

    if (!Array.isArray(rows) || rows.length === 0) {
      return res.status(400).json({
        error: 'rows must be a non-empty array with {row_num, col_count}'
      });
    }
    ...
    for (const row of rows) {
      if (!Number.isInteger(row.row_num) || !Number.isInteger(row.col_count)) {
        return res.status(400).json({
          error: 'Each row must have row_num and col_count as integers'
        });
      }
      await db.prepare(`
